...
type CBDShop = {
  ...
  email: string;
  address: string;
  price: string;            // coming from server as a number
  startDay: string;
  endDay: string;
...
type Form = Omit<CBDShop, "id"> & {
  price: string;            // now a string (e.g. "42,33")
};
...
export default function AdminCBDShopsContent() {
  ...
  const fetchShops = async () => {
    try {
      const res = await apiClient.get("/CBDShops", {
        params: { page: 1, pageSize: 50 }, headers
      });
      setShops(res.data.cbdShops.map((it: any) => ({
        ...it,
  ...
  // 👇 wrap fetchShops in a non-async callback to avoid returning a Promise
  useEffect(() => {
    fetchShops();
  }, []);
  ...
  const openModal = (it?: CBDShop) => {
    if (it) {
      ...
      setImagePreview(it.imagePath ? `${API_URL}/images/CBDShops/${it.imagePath}` : null);
      setCoverPreview(it.coverImagePath ? `${API_URL}/images/CBDShops/${it.coverImagePath}` : null);
      setImageFile(null);
      setCoverFile(null);
      setMode("edit");
    } else {
  ...
  // on submit (add or edit)
  const onSubmit = async (e: React.FormEvent) => {
    e.preventDefault();

    // normalize & round price once:
    ...
    try {
      if (mode === "add") {
        await apiClient.post("/CBDShops", fd, {
          headers: { ...headers, "Content-Type": "multipart/form-data" }
        });
      } else if (selected) {
        await apiClient.put(`/CBDShops/${selected.id}`, fd, {
          headers: { ...headers, "Content-Type": "multipart/form-data" }
        });
      }
      fetchShops();
          ...
          >
            ...
            <form onSubmit={onSubmit} className="space-y-4">
              ...
              <fieldset className="border p-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm">Tag von</label>
                    <select
                      name="startDay"
                      value={form.startDay}
                      onChange={onChange}
                  ...
                  <div>
                    <label className="block text-sm">Tag bis</label>
                    <select
                      name="endDay"
                      value={form.endDay}
                      onChange={onChange}
                ...
                <div className="grid grid-cols-2 gap-4 mt-2">
                  <div>
                    <label className="block text-sm">Zeit von</label>
                    <select
                      name="startTime"
                  ...
                  <div>
                    <label className="block text-sm">Zeit bis</label>
                    <select
                      name="endTime"
                      value={form.endTime}
                      onChange={onChange}
