...
export const ShopList: React.FC = () => {
  ...
  const fetchShops = async () => {
    ...
    try {
      setLoading(true);
      const response = await apiClient.get("/cbdshops", {
        params: {
      ...
      setShops(response.data.cbdShops || []);
  ...
  const sortedShops = useMemo(() => {
    ...
    else if (sortBy === "Z-A") s.sort((a, b) => b.name.localeCompare(a.name));
    return s;
  }, [shops, sortBy]);
  ...
  return (
    <div className="w-full h-screen flex flex-col">
      <HeaderWrapper className="text-white font-bold p-4">
        <h1 className="text-xl md:text-3xl">Cannabis Shops der Nähe</h1>
      </HeaderWrapper>
      ...
      <div className="flex flex-col md:flex-row flex-1 overflow-hidden gap-4">
        <div className="w-full md:w-1/3 p-4 overflow-y-auto">
          <Row className="pb-4 justify-center">
            <Col md={12}>
              <IconInput
                ...
                placeholder={
                  isAddressSearch ? "Suche nach Ort/Plz" : "Suche nach Shops"
                }
              />
            </Col>
          </Row>
          ...
          {loading ? (
            ...
            <div className="flex justify-center">Keine Shops gefunden</div>
          ) : (
