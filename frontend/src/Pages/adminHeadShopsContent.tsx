...
type Form = Omit<HeadShop, "id"> & {
  price: string;            // string for input, e.g. "42,33"
};
...
export default function AdminHeadShopsContent() {
  ...
  const fetchShops = async () => {
    try {
      const res = await apiClient.get("/HeadShops", {
        params: { page: 1, pageSize: 50 },
        headers,
      });
      setShops(
        res.data.headShops.map((it: any) => ({
          ...it,
  ...
  const openModal = (it?: HeadShop) => {
    if (it) {
      ...
      setImagePreview(
        it.imagePath
          ? `${API_URL}/images/HeadShops/${it.imagePath}`
          : null
      );
      setCoverPreview(
        it.coverImagePath
          ? `${API_URL}/images/HeadShops/${it.coverImagePath}`
          : null
      );
      setImageFile(null);
      setCoverFile(null);
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === "add") {
        await apiClient.post("/HeadShops", fd, {
          headers: { ...headers, "Content-Type": "multipart/form-data" },
        });
      } else if (selected) {
        await apiClient.put(`/HeadShops/${selected.id}`, fd, {
          headers: { ...headers, "Content-Type": "multipart/form-data" },
        });
      }
      fetchShops();
      ...
      console.error("Error saving head shop:", err);
      alert("Save failed");
    }
  };
  ...
  const onDelete = async (id: string) => {
    ...
    await apiClient.delete(`/HeadShops/${id}`, { headers });
    fetchShops();
  };
        ...
        {shops.map((s) => (
          ...
          >
            <div className="flex items-center gap-3">
              {s.imagePath && (
                <img
                  src={`${API_URL}/images/HeadShops/${s.imagePath}`}
                  alt={s.name}
